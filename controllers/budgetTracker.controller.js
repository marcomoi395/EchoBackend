...
const handleAddItem = async (req, res) => {
    ...
    */
    ...
    data.description = [{content: data.description}, {date: data.date}]
...
// POST budget-tracker/delete
const handlesyncBank = async (req, res) => {
    const record = req.body.payment;
    const userId = req.userId;

    try {
        const isExist = await BudgetTracker
            .findOne({
                userId: userId, transaction_id: record.transaction_id
            }).exec();

        if (!isExist) {
            const data = {
                userId: userId,
                descriptions: [{content: record.content, date: record.date}],
                amount: record.debitAmount === 0 ? record.creditAmount : record.debitAmount,
                type: record.debitAmount === 0 ? 'income' : 'expense',
                date: new Date(record.date),
                transaction_id: record.transaction_id,
                benAccountName: record.benAccountName,
                benAccountNo: record.benAccountNo,
...
module.exports = {handleGetItems, handleAddItem, handleDeleteItem, handlesyncBank}